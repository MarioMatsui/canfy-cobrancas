...
export function CheckoutClient({ publicToken }: { publicToken: string }) {
  ...
  useEffect(() => {
    if (state.kind !== 'success' || state.charge.orderStatus !== 'PENDING_PAYMENT') return;
    const timer = window.setInterval(() => { void refresh({ silent: true }); }, PENDING_REFRESH_MS);
  ...
  if (state.kind === 'temporary_error') return <CheckoutUnavailable kind="temporary_error" onRetry={() => void refresh()} />;
  if (state.kind === 'unexpected_response') return <CheckoutUnavailable kind="unexpected_response" onRetry={() => void refresh()} />;

  const { charge } = state;
  if (charge.orderStatus === 'PAID') return <CheckoutPaid charge={charge} />;
  if (charge.orderStatus === 'PENDING_PAYMENT') {
    return <PendingPayment charge={charge} refreshing={refreshing} refreshIssue={refreshIssue} onRefresh={() => void refresh()} />;
  }

  return <CheckoutShell main={<CheckoutFlow charge={charge} />} summary={<OrderSummary charge={charge} />} />;
}
